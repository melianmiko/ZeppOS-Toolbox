  ...
  render() {
    ...
    this.getObject("Time.TimeZone2", (obj) => {
      ...
      const view = hmUI.createWidget(hmUI.widget.TEXT_IMG, {
        ...
        show_level: hmUI.show_level.ONLY_NORMAL,
        dot_image: this.getFileFromID(this.getObject("Time.TimeZone2DelimiterImage")),
        ...this.convertAlignProps(obj)
      });

      updateHandlers.push(() => {
        let text = "";
        const info = weather.getForecastWeather().tideData;
        if(info.count > 0) {
          const data = info.data[0].sunrise;
          text = data.hour.toString().padStart(2, "0") + "." +
            data.minute.toString().padStart(2, "0");
        }
        view.setProperty(hmUI.prop.TEXT, text);
      })
    })
